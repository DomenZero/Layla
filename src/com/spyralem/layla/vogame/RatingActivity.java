...
//Iterator iter = temperatures.entrySet().iterator();

int lengthRow = 0;

TableLayout contentTable = (TableLayout) findViewById(R.id.rating_tableHeader);
...
//teView.setText(" User Name: "+" Rating: ");
//tableRow.addView(teView);	        		
//contentTable.addView(tableRow, rowParams);
//while (hasNext()) {

  // Map.Entry temperature = (Map.Entry) iter.next();

//
//   lengthRow = temperature.getKey().length() + temperature.getValue().length();
//
//   if (longestRow.isEmpty() || lengthRow > (longestRow.length()-1)) //Include -1 for subtracting the space occupied by "-"
//
//      longestRow = temperature.getKey() + "-" + temperature.getValue();
...
//		}
	...
	private TableLayout AddRowInTable(TableLayout tableLayout){//, String contentName, String contentRating){
